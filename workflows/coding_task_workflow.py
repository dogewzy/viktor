...
class CodingTaskWorkflow:
    def __init__(self) -> None:
        ...
        self._llm_retries = 0  # LLM 限流类失败的累计重试次数（成功推进一轮后清零）
    ...
    async def _run_loop(self, task_id: str) -> dict[str, Any]:
        while True:
            ...
            if status == "created":
                ...
                self._llm_retries = 0  # planning 成功产出 plan：限流重试预算复位
                ...
                await self._retry_rate_limited(task_id)

            elif status == "waiting_code_review":
                self._llm_retries = 0  # 一轮 attempt 成功产出 MR：限流重试预算复位
    ...
    async def _retry_rate_limited(self, task_id: str) -> None:
        """LLM 限流类失败的可恢复重试：指数退避（durable timer）后重派 attempt。

        超过 rate_limit_max_retries 仍限流 → 落 failed 终态（下轮主循环读到 failed 即返回）。
        退避期间监听 cancel：被取消则不重派，回主循环走 cancel 分支。
        """
        if self._llm_retries >= temporal_config.rate_limit_max_retries:
            await self._fail(
                task_id,
                f"LLM 持续限流，重试 {self._llm_retries} 次后仍失败",
            )
            return
        self._llm_retries += 1
        backoff = min(
            temporal_config.rate_limit_backoff_sec * (2 ** (self._llm_retries - 1)),
            temporal_config.rate_limit_backoff_max_sec,
        )
        await workflow.execute_activity(
            emit_task_event,
            args=[
                task_id, "rate_limit_backoff",
                f"LLM 限流，{backoff}s 后第 {self._llm_retries}/{temporal_config.rate_limit_max_retries} 次重试",
                {"backoff_sec": backoff, "attempt": self._llm_retries}, "rate_limited",
            ],
            start_to_close_timeout=_SHORT, retry_policy=_RETRY, schedule_to_close_timeout=_STC,
        )
        await workflow.sleep(timedelta(seconds=backoff))
        if self._cancel:
            return
        mode = await workflow.execute_activity(
            requeue_after_rate_limit, task_id,
            start_to_close_timeout=_DISPATCH, retry_policy=_RETRY, schedule_to_close_timeout=_STC,
        )
        await self._ensure_job(task_id, mode)
        await self._await_status_leaves(task_id, "planning" if mode == "planning" else "running")
