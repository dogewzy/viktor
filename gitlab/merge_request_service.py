...
def update_merge_request(
    *,
    ...
    token = gitlab_config.token_for_base_url(base_url)
    if not token:
        raise RuntimeError(f"缺少 GitLab Token，无法更新 {base_url} 的 Merge Request")
    project_path = GitLabClient.extract_project_path(repo_url)
    encoded = quote_plus(project_path)
    ...
    return _with_retry(_call, op_name="update_merge_request")
...
def set_commit_status(
    *,
    ...
    """Set GitLab commit status for staging gate.

    state: pending | running | success | failed | canceled | skipped.
    """
    base_url = gitlab_config.resolve_base_url(repo_url)
    token = gitlab_config.token_for_base_url(base_url)
    if not token:
        raise RuntimeError(f"缺少 GitLab Token，无法设置 {base_url} 的 commit status")
    project_path = GitLabClient.extract_project_path(repo_url)
    encoded = quote_plus(project_path)
    sha_q = quote_plus(str(sha))
    url = f"{GitLabClient.normalize_base_url(base_url)}/api/v4/projects/{encoded}/statuses/{sha_q}"
    ...
    return _with_retry(_call, op_name="set_commit_status")
...
def list_merge_request_notes(
    *,
    ...
    token = gitlab_config.token_for_base_url(base_url)
    if not token:
        raise RuntimeError(f"缺少 GitLab Token，无法查询 {base_url} 的 Merge Request 评论")
    project_path = GitLabClient.extract_project_path(repo_url)
    encoded = quote_plus(project_path)
    iid = quote_plus(str(merge_request_iid))
    url = f"{GitLabClient.normalize_base_url(base_url)}/api/v4/projects/{encoded}/merge_requests/{iid}/notes"
    ...
    def _call() -> list[dict[str, Any]]:
        with httpx.Client(headers={"PRIVATE-TOKEN": token}, timeout=30.0) as client:
            resp = client.get(url, params={"per_page": per_page, "sort": "desc", "order_by": "updated_at"})
            resp.raise_for_status()
            return resp.json()

    return _with_retry(_call, op_name="list_merge_request_notes")
...
def update_merge_request_note(
    *,
    ...
    token = gitlab_config.token_for_base_url(base_url)
    if not token:
        raise RuntimeError(f"缺少 GitLab Token，无法更新 {base_url} 的 Merge Request 评论")
    project_path = GitLabClient.extract_project_path(repo_url)
    encoded = quote_plus(project_path)
    iid = quote_plus(str(merge_request_iid))
    nid = quote_plus(str(note_id))
    url = f"{GitLabClient.normalize_base_url(base_url)}/api/v4/projects/{encoded}/merge_requests/{iid}/notes/{nid}"

    def _call() -> dict[str, Any]:
        with httpx.Client(headers={"PRIVATE-TOKEN": token}, timeout=30.0) as client:
            resp = client.put(url, json={"body": body})
            resp.raise_for_status()
            return resp.json()

    return _with_retry(_call, op_name="update_merge_request_note")


def create_or_update_merge_request_note(
    *,
    repo_url: str,
    merge_request_iid: int | str,
    marker: str,
    ...
    """Create or update a marker-based MR note."""
    notes = list_merge_request_notes(repo_url=repo_url, merge_request_iid=merge_request_iid)
    for note in notes:
        existing = str(note.get("body") or "")
        if marker and marker in existing:
            return update_merge_request_note(
                repo_url=repo_url,
                merge_request_iid=merge_request_iid,
                note_id=note.get("id"),
                body=body,
            )
    return create_merge_request_note(repo_url=repo_url, merge_request_iid=merge_request_iid, body=body)
