...
from core.temporal.client import (
    ...
    ISSUE_LINK_WORKFLOW,
    STAGING_COORDINATOR_WORKFLOW,
    get_temporal_client,
    link_workflow_id,
    staging_coordinator_workflow_id,
    task_workflow_id,
)
...
async def astart_staging_coordinator(env_id: str) -> bool:
    """启动（或复用）单测试环境 staging coordinator，并 signal 它检查队列。"""
    if not enabled():
        return False
    client = await get_temporal_client()
    workflow_id = staging_coordinator_workflow_id(env_id)
    try:
        await client.start_workflow(
            STAGING_COORDINATOR_WORKFLOW,
            args=[env_id],
    ...
    await handle.signal("run_queued")
    return True
...
def start_staging_coordinator_sync(env_id: str) -> bool:
    if not enabled():
        return False
    return _run_sync(astart_staging_coordinator(env_id))
