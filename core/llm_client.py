...
class LLMRouter(BaseChatModel):
        ...
        for attempt_index, provider_id in enumerate(self._ordered_provider_ids(), start=1):
            ...
            try:
                ...
                async for chunk in self._stream_provider_async(model, messages, stop=stop, run_manager=run_manager, **kwargs):
                    ...
                    chunk_usage = _chunk_usage_metadata(chunk)
                    if chunk_usage:
    ...
    def _build_provider_model(self, provider_id: str) -> BaseChatModel:
        ...
        kwargs: dict[str, Any] = {
            ...
            # 自定义 base_url 会关闭 langchain 的 usage 自动开启，显式打开以拿到流式真实 token。
            "stream_usage": True,
        }
    ...
    def _record_from_stream(
        ...
        *,
        ...
        """流式收尾：优先用最后一个 chunk 的真实 usage，否则回退到估算。"""
        structure: dict[str, int | None] = {
        ...
        if usage_metadata:
            prompt_tokens = _as_int(usage_metadata.get("input_tokens")) or _estimated_message_tokens(messages)
            completion_tokens = _as_int(usage_metadata.get("output_tokens")) or _estimated_output_tokens(output_chars)
            total_tokens = _as_int(usage_metadata.get("total_tokens")) or (prompt_tokens + completion_tokens)
            structure = _token_structure_from_usage({}, usage_metadata)
            token_count_source = "actual"
            tps = _token_tps(completion_tokens, duration_ms) or _estimated_tps(output_chars, duration_ms)
        else:
            ...
            tps = _estimated_tps(output_chars, duration_ms)
        return LLMCallRecord(
            request_id=request_id,
            feature=self.feature,
            provider=provider_id,
            ...
            completion_tokens=completion_tokens,
            total_tokens=total_tokens,
            cache_hit_tokens=structure.get("cache_hit_tokens"),
            cache_miss_tokens=structure.get("cache_miss_tokens"),
            reasoning_tokens=structure.get("reasoning_tokens"),
            output_tokens=structure.get("output_tokens"),
            output_chars=output_chars,
            tokens_per_second=tps,
            meta=_record_meta(
                token_count_source=token_count_source,
...
def is_rate_limit_error(error: Exception) -> bool:
    ...
    仅含 429/TPM 限流（容量瞬时打满，等窗口 reset 后可恢复）；不含 402/欠费/额度耗尽
    （那是账务问题，重试无意义，应直接落 failed）。
    """
    return _is_rate_limit(error)
...
def _usage_from_result(result: ChatResult) -> dict[str, int]:
    ...
    if result.generations:
        ...
        if isinstance(raw, dict):
            usage_metadata = raw
    # 合并视图：原始 OpenAI 风格 usage + LangChain 归一化 usage_metadata。
    usage: dict[str, Any] = dict(raw_usage)
    if usage_metadata:
    ...
    # 追加 token 结构（缓存命中/未命中、思考、可见输出）。
    result_usage.update(_token_structure_from_usage(raw_usage, usage_metadata))
    return result_usage


def _as_int(value: Any) -> int | None:
    """容错地把数值转 int；非数值返回 None。"""
...
def _nested_dict(container: Any, key: str) -> dict[str, Any]:
    """仅当 container 是 dict 且 container[key] 也是 dict 时返回它，否则返回空 dict。"""
...
def _token_structure_from_usage(raw_usage: dict, usage_metadata: dict | None) -> dict[str, int | None]:
    """从原始/归一化 usage 中提取 token 结构。

    返回 cache_hit_tokens / cache_miss_tokens / reasoning_tokens / output_tokens，
    未知一律返回 None（区分“无数据”与“确为 0”）。
    """
    raw = raw_usage if isinstance(raw_usage, dict) else {}
    meta = usage_metadata if isinstance(usage_metadata, dict) else {}

    prompt_tokens = _as_int(raw.get("prompt_tokens") or raw.get("input_tokens"))
    completion_tokens = _as_int(raw.get("completion_tokens") or raw.get("output_tokens"))

    completion_details = _nested_dict(raw, "completion_tokens_details")
    prompt_details = _nested_dict(raw, "prompt_tokens_details")
    meta_input_details = _nested_dict(meta, "input_token_details")
    meta_output_details = _nested_dict(meta, "output_token_details")

    # cache_hit：DeepSeek 顶层 → OpenAI prompt_tokens_details.cached_tokens → usage_metadata.cache_read
    cache_hit = _as_int(raw.get("prompt_cache_hit_tokens"))
    if cache_hit is None:
        cache_hit = _as_int(prompt_details.get("cached_tokens"))
    if cache_hit is None:
        cache_hit = _as_int(meta_input_details.get("cache_read"))

    # cache_miss：DeepSeek 顶层 → prompt_tokens - cache_hit（两者已知时）
    ...
    # reasoning：completion_tokens_details.reasoning_tokens → usage_metadata.output_token_details.reasoning
    reasoning = _as_int(completion_details.get("reasoning_tokens"))
    if reasoning is None:
        reasoning = _as_int(meta_output_details.get("reasoning"))

    # 可见 output = completion - reasoning（两者已知时）；否则退化为 completion；再否则 None
    if completion_tokens is not None and reasoning is not None:
        output_tokens: int | None = completion_tokens - reasoning
...
def _chunk_usage_metadata(chunk: ChatGenerationChunk) -> dict[str, Any] | None:
    """从流式 chunk 取 usage_metadata（通常只有最后一个 chunk 带）。"""
    message = getattr(chunk, "message", None)
    raw = getattr(message, "usage_metadata", None)
