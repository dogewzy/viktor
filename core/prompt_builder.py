...
# Skill 渐进式披露：命中后注入的 skill 数上限，以及全量展开的条数。
# 其余命中项只给一行索引，LLM 需要完整步骤时用 expand_skill 工具按需拉取。
_SKILL_INJECT_TOPN = 8
_SKILL_FULL_EXPAND = 1
...
def render_skill_full(skill) -> list[str]:
    """渲染单个 skill 的完整方法体（描述/触发样例/依赖/推荐工具/步骤/输出要求）。

    既用于 prompt 内"第一名全量展开"，也用于 expand_skill 工具按需拉取，保证两处呈现一致。
    """
    lines = [f"### {skill.name} ({skill.id}, kind={skill.kind})"]
...
def _build_skills_block(project_id: str, user_message: str = "") -> tuple[str, list[str]]:
    """拼装项目 Skill，作为方法层注入 prompt（混合两层渐进披露）。

    用 BM25 + 触发样例命中提权对 skill 排序；最高分若干条全量展开，其余命中项只给一行
    索引（name + 触发摘要），LLM 需要完整步骤时用 expand_skill 工具按需拉取。

    Returns:
        (block 文本, 已全量展开的 skill id 列表)
    """
    scored = retrieve_skills(project_id, user_message, limit=_SKILL_INJECT_TOPN)
    if not scored:
        return "", []

    selected = [item for item, _ in scored]
    expanded = selected[:_SKILL_FULL_EXPAND]
    indexed = selected[_SKILL_FULL_EXPAND:]
    expanded_ids = [s.id for s in expanded]

    lines = ["\n## 项目技能 Skill（方法层：什么时候按什么流程调用上下文和工具）"]
    for skill in expanded:
        lines.append("")
        lines.extend(render_skill_full(skill))

    if indexed:
        lines.append("\n### 其它相关 Skill（仅索引，需要完整步骤时调用 expand_skill(skill_id) 拉取）")
        for skill in indexed:
            summary = ""
            for ex in skill.trigger_examples:
                if ex.text:
                    summary = ex.text
                    break
            summary = summary or skill.description
            tail = f" — {summary}" if summary else ""
            lines.append(f"- {skill.name} ({skill.id}){tail}")
...
async def build_system_prompt(
    ...
    skills_block, _expanded_skill_ids = _build_skills_block(project_id, user_message)
    if skills_block:
