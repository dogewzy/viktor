...
def prepare_link_blueprint(link_id: str) -> dict[str, Any]:
    ...
    current = _as_dict(link.result)
    if not _as_dict(current.get("test_plan")):
        bp["test_plan"] = _default_staging_test_plan(link)
    else:
        bp["test_plan"] = current.get("test_plan")
...
def apply_link_blueprint(
    ...
    normalized_test_plan = _normalize_test_plan(test_plan or prev_bp.get("test_plan") or _default_staging_test_plan(link))
    bp = {
        "repos": routed,
    ...
    with _RESULT_LOCK:
        ...
        try:
            row = db.get(IssueIntakeLinkModel, link_id)
            if row:
                row.result = {**(row.result or {}), "routed": routed, "blueprint": bp, "test_plan": normalized_test_plan}
                db.commit()
        finally:
            db.close()
    emit_issue_event(
...
def _default_staging_test_plan(link: IssueIntakeLinkModel) -> dict[str, Any]:
    """生成一份保守的 staging 测试计划草案；人工可在 blueprint review 时替换。"""
    title = (link.title or "核心用户路径").strip()
    return {
        "version": 1,
        "review_status": "pending",
        "summary": f"围绕「{title}」执行浏览器端到端验收；如未配置自动化映射，由 Playwright 总入口决定实际用例。",
        "cases": [
            {
                "id": "staging_smoke_001",
                "title": title[:80] or "核心流程冒烟",
                "type": "e2e",
                "priority": "critical",
                "steps": [],
                "expected": [],
                "automation": {"runner": "playwright", "file": "", "grep": "", "tags": []},
...
def _normalize_test_plan(value: dict[str, Any]) -> dict[str, Any]:
    plan = dict(value or {})
    plan["version"] = int(plan.get("version") or 1)
    plan["review_status"] = "approved"
    cases = []
    for idx, item in enumerate(_as_list(plan.get("cases"))):
        if not isinstance(item, dict):
            continue
        cid = str(item.get("id") or f"case_{idx + 1:03d}").strip()
        cases.append({
            "id": cid,
            "title": str(item.get("title") or cid).strip(),
            "type": str(item.get("type") or "e2e").strip(),
            "priority": str(item.get("priority") or "normal").strip(),
            "steps": _as_list(item.get("steps")),
            "expected": _as_list(item.get("expected")),
            "automation": _as_dict(item.get("automation")),
...
def _mark_mr_ready(link_id: str, task: dict[str, Any]) -> None:
    ...
    if mr_url:
        ...
        try:
            ...
            logger.warning("[issue-intake] staging enqueue failed link={}: {}", link_id, e)
