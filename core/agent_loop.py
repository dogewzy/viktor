...
from core.prompt_builder import build_system_prompt, render_skill_full
...
def _expand_skill(project_id: str, skill_id: str) -> str:
    """按需拉取单个 Skill 的完整方法体（配合 prompt 内的 Skill 索引使用）。"""
    skill = registry.get_skill(project_id, (skill_id or "").strip())
    if skill is None or skill.status != "enabled":
        available = ", ".join(s.id for s in registry.get_skills(project_id, only_enabled=True)) or "（无）"
        return (
            f"未找到可用 Skill：{skill_id!r}。请使用系统 prompt「项目技能 Skill」索引里列出的合法 id。"
            f"\n当前 enabled skill: {available}"
        )
    return "\n".join(render_skill_full(skill))


def _build_skill_tools(project_id: str) -> list[StructuredTool]:
    """expand_skill：按需展开 Skill 索引里只给了摘要的 skill。

    仅当项目存在 enabled skill 时才暴露该工具（无能力则不暴露，与代码自省工具一致）。
    """
    if not registry.get_skills(project_id, only_enabled=True):
        return []

    ExpandSkillArgs = create_model(
        "ExpandSkillArgs",
        skill_id=(str, Field(description="要展开的 Skill ID（取自系统 prompt「项目技能 Skill」索引）")),
    )
    return [
        StructuredTool.from_function(
            func=lambda skill_id: _expand_skill(project_id, skill_id),
            name="expand_skill",
            description=(
                "当系统 prompt 的「项目技能 Skill」索引里某个 skill 与当前问题相关、但只给了一行摘要时，"
                "用它按 skill_id 拉取该 skill 的完整执行步骤、依赖上下文、推荐工具与输出要求。"
                "已全量展开在 prompt 里的 skill 无需再调用。"
            ),
            args_schema=ExpandSkillArgs,
        )
    ]
...
def _build_project_tools(project_id: str, topic_thread_id: str = "") -> list[StructuredTool]:
    ...
    顺序：文件导出 -> 运行时上下文 -> 数据库探索工具 -> 日志 -> 外部连接器 -> 代码自省 -> K8s 内置工具 -> Skill 展开。
    """
    ...
    tools.extend(_build_skill_tools(project_id))
    return tools
