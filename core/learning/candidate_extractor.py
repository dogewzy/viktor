...
7. evaluation_summary / Ragas 分数只能作为质量诊断信号，不能把一次分数或一次失败沉淀成长期事实。
8. skill payload 只能用 SkillItem 的真实字段，禁止自创字段名（如 steps、anti_patterns、preconditions、sql_template、triggers 都是错的）。必须遵守下面的字段映射：
   - 可执行步骤写进 "instructions"（字符串数组，按顺序），不要写进 steps；
   - 典型问法写进 "trigger_examples"（每项 {{text, source, confirmed, confidence}}），不要写进 triggers；
   - 需要用到的工具 id 写进 "required_tools"（字符串数组）；
   - 需要的上下文写进 "required_contexts"（每项 {{type, id, name, required, purpose}}）；
   - 前置条件 / 反模式 / SQL 模板等没有对应字段的内容，统一并入 instructions 或 description 文字描述，不要新增键。
...
输出格式：
{
  "candidates": [
    ...
    {
      "target_type": "skill",
      "target_id": "url-md5-query-media",
      "title": "技能：按 URL 列表查 media 表走 link_md5 索引",
      "content": "按 URL 列表查 media 表时直接 IN 会全表扫描超时，必须先算 md5 再走 link_md5 索引。",
      "payload": {
        "name": "按 URL 列表查 media 表",
        "kind": "operational",
        "description": "按 URL 列表查 media 表的可复用流程，避免全表扫描被拦截。",
        "trigger_examples": [{"text": "帮我按这批链接查 media 表", "source": "llm_expanded", "confirmed": false, "confidence": 0.7}],
        "required_tools": ["execute_sql"],
        "required_contexts": [{"type": "database", "id": "", "name": "media 库", "required": true, "purpose": "执行只读查询"}],
        "instructions": [
          "用 hashlib.md5(url.encode()).hexdigest() 批量算出每个 URL 的 md5",
          "用 link_md5 IN (...) 查 media 表，禁止直接对 link/clip_url 做 IN（无索引会全表扫描）",
          "按需用结果关联其他表"
        ],
        "input_contract": {"signals": ["一批 URL/侵权链接"]},
        "output_contract": {"sections": ["结论", "证据"], "must_cite_evidence": true},
        "safety_policy": {"readonly_only": true},
        "enabled": true
      },
      "confidence": 0.8,
      "risk_level": "low",
      "evidence_event_seq": [4, 7]
    }
  ]
}
""".strip()
    ...
    if digest.repeated_tools or digest.sql_blocked_events or digest.connector_failures:
        ...
        drafts.append(LearningCandidateDraft(
            ...
            payload={
                ...
                "enabled": True,
                "source": "manual",
                "evaluation_signal": digest.evaluation_summary or {},
            },
            confidence=0.5,
            risk_level="medium",
